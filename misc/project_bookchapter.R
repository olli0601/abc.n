ch11.mutost<- function()
{
	...
	#	type I error	
	ans <- mutost.calibrate(n.of.y=length(sim), s.of.y=sd(sim), tau.u=0.8, what='CR', alpha=0.01, plot=TRUE)		
	...
	accprob	<- mutost.pow(rho, df=length(sim), s.of.T=s.of.T, c.u=0.2)
	...
	ggsave(file, w=5, h=5)
	#
	mutost.calibrate(n.of.y=length(sim), s.of.y=sd(sim), tau.u=0.23, what='CR', alpha=0.01, plot=TRUE)
