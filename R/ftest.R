...
#' @description Compute the power of the two-sample multivariate equivalence test for population means of multivariate normal summary values with unknown population variance.
#' @export
...
ftest.pow <- function(rho2, tau2, alpha = 0.01, n, m, p, support = c(0, Inf), log = FALSE)
{ 
	stopifnot(alpha > 0, alpha < 0.5, support[1] <= support[2], tau2 > 0, support[1] >= 0)
	stopifnot(n %% 1 == 0, m %% 1 == 0, p %% 1 == 0, n > 0, m > 0, p > 0, p < (n + m - 1))
	ans 			<- rho2
	in_support 		<- (rho2 >= support[1] & rho2 <= support[2])
	ans[!in_support]<- ifelse(log, -Inf, 0)
	if(any(in_support))
	{
	    crit.val            <- qf(alpha, p, n + m - p - 1, n * m * tau2 / (n + m))
		ans[in_support]		<- pf(crit.val, p, n + m - p - 1, n * m * rho2[in_support] / (n + m))
...
ftest.criticalvalue <- function(tau2, n, m, p, alpha = 0.01)
{				 
	c <- p * ( n + m - 2) * qf(alpha, p, n + m - p - 1, n * m * tau2 / (n + m)) / (n + m - p - 1)
	names(c) <- "c"
	c
}
#------------------------------------------------------------------------------------------------------------------------
# Calibrate the equivalence region for given maximum power
ftest.calibrate.tolerances <- function(mx.pw, n, m, p, tau2.ub, alpha = 0.01, tol = 1e-5, max.it = 100, verbose = 0)
{
	...
	tau2.lb <- 0
	tmp	<- max.it
	while(ftest.pow(0, tau2.ub, alpha, n, m, p) < pw.ub & tmp > 0)
	{
	    tau2.ub <- tau2.ub + 1
	    tmp <- tmp - 1
	}
	...
	tmp	<- max.it
	while(ftest.pow(0, tau2, alpha, n, m, p) > pw.lb & tmp > 0)
	{
	    tau2 <- (tau2 + tau2.lb) / 2
	    tmp <- tmp - 1
	}
	...
	if(ftest.pow(0, tau2.lb, alpha, n, m, p) > ftest.pow(0, tau2.ub, alpha, n, m, p)) stop("non-viable bounds generated")
	
	tmp	<- max.it
	while(tmp > 0 & round(tau2.lb, digits = 10) != round(tau2.ub, digits = 10))
	{
	    tau2 <- seq(tau2.lb, tau2.ub, len = 1024)
	    curr.pw <- sapply(tau2, function(x) ftest.pow(0, x, alpha, n, m, p))
		...
		if(verbose)	cat(paste("\ntrial upper bound", tau2.ub, "with power", ftest.pow(0, tau2.ub, alpha, n, m, p), " lower bound", tau2.lb, "with power", ftest.pow(0, tau2.lb, alpha, n, m, p)))
		tmp <- tmp - 1
	}
	if(tmp == 0)	stop("could not find tau2")
	tau2 <- round(tau2.lb, digits = 10)
	curr.pw <- ftest.pow(0, tau2, alpha, n, m, p)
	if(verbose)	cat(paste("\nFound tau2", tau2, "with power", curr.pw, "at rho=0\n"))
	ans				<- c(tau2, curr.pw, abs(curr.pw - mx.pw))	
	names(ans)		<- c("tau2", "curr.pw", "error")
	c(ans, ftest.criticalvalue(tau2 = tau2, n = n, m = m, p = p, alpha = alpha))
}

#------------------------------------------------------------------------------------------------------------------------
ftest.plot <- function(n, m, p, tau2, alpha, pow_scale = 1.5)
{
	tmp			<- data.frame(rho2 = seq(0, pow_scale * tau2, length.out = 1024))	
	tmp$power	<- ftest.pow(tmp$rho2, tau2, alpha, n, m, p)
	...
	p	<- ggplot(tmp, aes(x = rho2, y = power)) + geom_line() + labs(x = expression(rho^2), y = 'Power\n(ABC acceptance probability)') +
			...
			ggtitle(substitute(paste("n = ", n, ", m = ", m, ", p = ", p, ", ", tau^2, " = ", tau2, sep = ""), list(n = n, m = m, p = p, tau2 = tau2)))
	print(p)
}

#------------------------------------------------------------------------------------------------------------------------
#' @title Calibrating \code{ftest} for ABC
#' @description Calibrate the two-sample equivalence test for population means of multivariate normal summary values with unknown population covariance matrix for ABC inference.
#' 
...
#' @param n 		Number of samples in observed data
#' @param m 		Number of samples in simulations
...
#' @references  http://arxiv.org/abs/1305.4283
ftest.calibrate<- function(n = NA, m = NA, p = NA, what = 'MXPW', 
							...
							max.it = 100, pow_scale = 1.5, plot = FALSE, verbose = FALSE)
{
	...
	if(what == 'ALPHA')
	{
		...
		stopifnot(n %% 1 == 0, m %% 1 == 0, p %% 1 == 0, n > 0, m > 0, p > 0, p < (n + m - 1))
		ans         <- pf((n + m - p - 1) * c / (p * (n + m - 2)), p, n + m - p - 1, n * m * tau2 / (n + m))
		names(ans)	<- c('alpha')
		if(plot)
			ftest.plot(n = n, m = m, p = p, tau2 = tau2, alpha = ans['alpha'], pow_scale = pow_scale)
	}
	if(what == 'CR')
	{
		stopifnot(tau2 > 0, alpha > 0, alpha < 1)
		stopifnot(n %% 1 == 0, m %% 1 == 0, p %% 1 == 0, n > 0, m > 0, p > 0, p < (n + m - 1))
		ans			<- ftest.criticalvalue(tau2 = tau2, n = n, m = m, p = p, alpha = alpha)
		names(ans)	<- c('c')
		if(plot)
			ftest.plot(n = n, m = m, p = p, tau2 = tau2, alpha = alpha, pow_scale = pow_scale)
	}
	if(what == 'MXPW')
	{
		stopifnot(mx.pw > 0, mx.pw < 1, alpha > 0, alpha < 0.5, max.it > 10, tol < 0.01, pow_scale > 1)
		stopifnot(n %% 1 == 0, m %% 1 == 0, p %% 1 == 0, n > 0, m > 0, p > 0, p < (n + m - 1))
		tmp			<- ftest.calibrate.tolerances(mx.pw, n = n, m = m, p = p, alpha = alpha, tau2.ub = ifelse(is.na(tau2), 1, tau2), tol = tol, max.it = max.it, verbose = verbose)
		ans			<- c(tmp[4], tmp[1], tmp[2], tmp[3])
		names(ans)	<- c('c','tau2', 'pw.cmx', 'pw.error')
		if(plot)
			ftest.plot(n = n, m = m, p = p, tau2 = ans['tau2'], alpha = alpha, pow_scale = pow_scale)		
	}	
	ans	
}
