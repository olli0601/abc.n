...
nabc_dratio_propwgausskernel <- function(support, ctheta, ptheta, covmat, give_log = F) {
	...
	if (is.diag(covmat)) {
		...
		if (give_log) {
			...
			r <- sum(log(pnorm(support["upper", names.sd.pos], mean = ctheta[names.sd.pos], sd = sigma[names.sd.pos]) - 
				pnorm(support["lower", names.sd.pos], mean = ctheta[names.sd.pos], sd = sigma[names.sd.pos])) - log(pnorm(support["upper", 
				names.sd.pos], mean = ptheta[names.sd.pos], sd = sigma[names.sd.pos]) - pnorm(support["lower", names.sd.pos], 
				mean = ptheta[names.sd.pos], sd = sigma[names.sd.pos])))
		} else {
			#take ratios for each model parameter to help with numerical accuracy
			r <- prod((pnorm(support["upper", names.sd.pos], mean = ctheta[names.sd.pos], sd = sigma[names.sd.pos]) - 
				pnorm(support["lower", names.sd.pos], mean = ctheta[names.sd.pos], sd = sigma[names.sd.pos]))/(pnorm(support["upper", 
				names.sd.pos], mean = ptheta[names.sd.pos], sd = sigma[names.sd.pos]) - pnorm(support["lower", names.sd.pos], 
				mean = ptheta[names.sd.pos], sd = sigma[names.sd.pos])))
		}
	} else {
		...
		if (give_log) {
			r <- log(pmvnorm(lower = support["lower", names.posdef], upper = support["upper", names.posdef], mean = ctheta[names.posdef], 
				sigma = covmat[names.posdef, names.posdef])) - log(pmvnorm(lower = support["lower", names.posdef], upper = support["upper", 
				names.posdef], mean = ptheta[names.posdef], sigma = covmat[names.posdef, names.posdef]))

		} else {
			r <- pmvnorm(lower = support["lower", names.posdef], upper = support["upper", names.posdef], mean = ctheta[names.posdef], 
				sigma = covmat[names.posdef, names.posdef])/pmvnorm(lower = support["lower", names.posdef], upper = support["upper", 
				names.posdef], mean = ptheta[names.posdef], sigma = covmat[names.posdef, names.posdef])
...
nabc_MA1_MCMC_MH <- function(data=NULL,theta_init=NULL,covmat_mvn_proposal=NULL,mcmc=NULL,a_true=0.1,sig2_true=1,eps_0_true=NULL,n_x=1000,a_bounds = c(-0.3, 0.3), sig2_bounds = c(0.5, 2), prior_dist=c("uniform","uniform_on_rho"),n_iter=1000,iter_adapt=n_iter,beta_adapt=0.5,plot=FALSE,dir_pdf=NULL,sample_from_prior= FALSE){
	...
	return(list(data=data,theta_init= theta_init,bounds=list(a=a_bounds,sig2=sig2_bounds),posterior= posterior,covmat_mvn_proposal= covmat_mvn_proposal,covmat_mvn_proposal_adapted= covmat_mvn_proposal_adapted))	
