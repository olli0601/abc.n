...
corrz.calibrate.tolerances.getkl <- function(s.of.x, s.of.T, tau.u, mx.pw=0.9, alpha=0.01, pow_scale=2, debug = 0, calibrate.tau.u=T, plot = F, verbose=0) 
{
	...
	tmp 		<- integrate(kl.integrand, lower=support[1], upper=support[2], dP=corrz.sulkl, dQ=corrz.pow, P_arg=lkl.arg, Q_arg=pw.arg)	
...
{	
	...
	tmp 				<- optimize(kl.optimize, interval = n2s(c(yn.ub, yn.lb)), x_name="s.of.T", is_integer=F, KL_divergence = "corrz.calibrate.tolerances.getkl", KL_args=KL_args, verbose=debug, tol=1e-5)	
